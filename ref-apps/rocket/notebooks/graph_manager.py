...
class GraphManager:
    ...
    def GraphTopologySet(self, opParams):
        ...
        if opParams.get('topologyUrl') is not None:
            ...
            path1 = str(pathlib.Path(__file__).parent.absolute())
            path2 = str(opParams['topologyFile'])
            fpath = path.join(path1, path2)
            topologyJsonString = fpath
        else:
            logging.info('Neither topologyUrl nor topologyFile specified')

        with open(topologyJsonString) as jsonfile:
            topologyJson = json.load(jsonfile)
